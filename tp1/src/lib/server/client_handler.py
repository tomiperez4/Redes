...
class ClientHandler(threading.Thread):
    ...
    def run(self):
        ...
        self.log.info(
            f"Client {self.address} requested operation {op_type} on {filename} ({file_size} bytes)"
        )
        file_manager = FileManager(
            self.protocol,
            self.log.clone("CLIENT (FILE MANAGER)"),
        ...
        if op_type == CLIENT_TYPE_DOWNLOAD:
            ...
            response_seg = struct.pack(RES_FORMAT, CODE_READY, size)
            ...
            if not valid:
                return
            response_seg = struct.pack(RES_FORMAT, CODE_READY, 0)
            self.protocol.send(response_seg)
    ...
    def _validate_upload(self, file_size, filename):
        ...
        if file_size > MAX_FILE_SIZE:
            self.log.error(
                f"File too large. Maximum file size is {MAX_FILE_SIZE / (1024 * 1024)} MB"
            )
        ...
        if self._is_storage_full(net_size):
            self.log.error("Not enough storage to upload.")
    ...
    def _end_conn(self, status_code):
        ...
        """
        response_seg = struct.pack(RES_FORMAT, status_code, 0)
        self.protocol.send(response_seg)
        self.on_finish_callback(self.address)

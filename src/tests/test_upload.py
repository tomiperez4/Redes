import os
import pytest
from conftest import SERVER_STORAGE, CLIENT_STORAGE, BASE_FILE, stored_name, run_upload, md5, wait_for_file
...
def test_upload(server, protocol):
    ...
    assert wait_for_file(stored), \
        f"File does not appear in server storage [{protocol}]"
    assert os.path.getsize(stored) == os.path.getsize(original), \
        f"Incorrect size [{protocol}]"
    assert md5(original) == md5(stored), \

...
from conftest import (
    SERVER_STORAGE, CLIENT_STORAGE, BASE_FILE,
    stored_name, run_upload, run_download, md5, wait_for_file
)
...
def test_concurrent_upload_and_download(server):
    ...
    for _, proto in tasks[:2]:
        ...
        assert wait_for_file(stored), \
            f"File not found in server storage [{proto}]"
        assert md5(original_client) == md5(stored), \
    ...
    for _, proto in tasks[2:]:
        ...
        assert wait_for_file(downloaded), \
            f"File not found in client storage [{proto}]"
        assert md5(original_server) == md5(downloaded), \
